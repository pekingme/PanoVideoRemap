...
Camera::Camera ( const string& name, const double u0, const double v0, const vector< double >& affine,
                 const vector< double >& poly, const vector< double >& inv_poly, const vector< double >& extrinsic )
    : _name ( name ), _u0 ( u0 ), _v0 ( v0 ), _c ( affine[0] ), _d ( affine[1] ), _e ( affine[2] ), _poly ( poly ), _inverse_poly ( inv_poly )
{
...
Mat Camera::ProjectWorldToFrame ( const Mat& world_pts, const bool debug ) const
{
    ...
    Mat camera_pts_n_4 = world_pts_n_4 * _transform_4_4.t();
